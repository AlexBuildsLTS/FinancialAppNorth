...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const response = await fetch(`${API_URL}?key=${apiKey}`, {
      ...
      body: JSON.stringify({
        system_instruction: {
          parts: [{ text: "You are NorthFinance CFO AI. Analyze financial data objectively. You are a tool, not a regulated financial advisor. Provide concise, professional, and actionable insights." }]
        },
