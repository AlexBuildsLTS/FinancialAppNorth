...
export const ChartSection = () => {
    ...
    const expenseData = useMemo(() => {
        ...
        const expenseSummary = transactions
            ...
            .reduce((acc: { [x: string]: { amount: number; }; }, transaction: { category: string; amount: number; }) => {
        ...
        return (Object.values(expenseSummary) as { category: string; amount: number }[]).map((item) => ({
            ...item,
             ...
             <Animated.View entering={FadeInUp.duration(500).delay(200)}>
                <Card>
                    ...
                    <View style={styles.noDataContainer}>
                       <Text style={{color: colors.textSecondary}}>No expense data available for the chart.</Text>
                    </View>
                </Card>
            </Animated.View>
        )
            ...
            <Card>
                ...
                <View style={styles.chartContainer}>
                    <VictoryPie
                        ...
                        labels={({ datum }) => `${datum.category}: $${datum.amount.toFixed(2)}`}
                        events={[{
                            target: 'data',
                            eventHandlers: {
                                onPressIn: (_, { index }) => {
                        ...
                        style={{
                            data: {
                                fillOpacity: ({ index }) => (activeSlice === null || activeSlice === index ? 1 : 0.6),
                            },
                        }}
                    />
                    <VictoryLabel
                        textAnchor="middle"
                        style={[{ fontSize: 20, fill: colors.text, fontWeight: 'bold' }, { fontSize: 14, fill: colors.textSecondary }]}
                        x={(screenWidth * 0.8) / 2}
                        y={125}
                ...
                <View style={styles.legendContainer}>
                    {expenseData.map(item => (
                        <View key={item.category} style={styles.legendItem}>
                            ...
                            <Text style={[styles.legendText, { color: colors.textSecondary }]}>{item.category}</Text>
                        </View>
                    ))}
                </View>
            </Card>
