...
export default function JournalEntryModal({ visible, onClose, onSuccess }: JournalEntryModalProps) {
  ...
  const [entries, setEntries] = useState<Omit<JournalEntryLine, 'id'>[]>([
    { accountId: '', accountName: '', accountCode: '', description: '', debitAmount: 0, creditAmount: 0 },
    { accountId: '', accountName: '', accountCode: '', description: '', debitAmount: 0, creditAmount: 0 }
  ]);
  ...
  const generateReference = () => {
    const now = new Date();
    const ref = `JE-${now.getFullYear()}${(now.getMonth() + 1).toString().padStart(2, '0')}${now.getDate().toString().padStart(2, '0')}-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`;
    setReference(ref);
  };
  ...
  const getTotalDebits = () => entries.reduce((sum, entry) => sum + (entry.debitAmount || 0), 0);
  const getTotalCredits = () => entries.reduce((sum, entry) => sum + (entry.creditAmount || 0), 0);
  const isBalanced = () => Math.abs(getTotalDebits() - getTotalCredits()) < 0.01;

  const handleSave = async () => {
    if (!reference || !description) {
      Alert.alert('Missing Fields', 'Please fill in Reference and Description.');
      return;
    }
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <Text style={[styles.sectionTitle, { color: colors.text }]}>Account Lines</Text>
              <TouchableOpacity onPress={addEntryLine} style={styles.addLineButton}>
                <Plus size={20} color={colors.primary} />
              </TouchableOpacity>
            </View>

            {entries.map((entry, index) => (
              <View key={index} style={[styles.entryLine, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                ...
                <View style={styles.row}>
                  <View style={styles.halfWidth}>
                    <Text style={[styles.label, { color: colors.textSecondary }]}>Debit</Text>
                    <TextInput
                      style={[styles.input, { color: colors.text, borderColor: colors.border, backgroundColor: colors.background }]}
                      value={entry.debitAmount > 0 ? entry.debitAmount.toString() : ''}
                      onChangeText={(value) => updateEntry(index, 'debitAmount', parseFloat(value) || 0)}
                      placeholder="0.00"
                      placeholderTextColor={colors.textSecondary}
                  ...
                  <View style={styles.halfWidth}>
                    <Text style={[styles.label, { color: colors.textSecondary }]}>Credit</Text>
                    <TextInput
                      style={[styles.input, { color: colors.text, borderColor: colors.border, backgroundColor: colors.background }]}
                      value={entry.creditAmount > 0 ? entry.creditAmount.toString() : ''}
                      onChangeText={(value) => updateEntry(index, 'creditAmount', parseFloat(value) || 0)}
                      placeholder="0.00"
                      placeholderTextColor={colors.textSecondary}
                      keyboardType="numeric"
                    />
          ...
          <View style={[styles.totalsSection, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <View style={styles.totalsHeader}>
              <Calculator size={20} color={colors.primary} />
              <Text style={[styles.totalsTitle, { color: colors.text }]}>Entry Totals</Text>
            </View>
            <View style={styles.totalsRow}>
              <Text style={[styles.totalsLabel, { color: colors.textSecondary }]}>Total Debits:</Text>
              <Text style={[styles.totalsValue, { color: colors.text }]}>
                ${getTotalDebits().toFixed(2)}
              </Text>
            </View>
            <View style={styles.totalsRow}>
              <Text style={[styles.totalsLabel, { color: colors.textSecondary }]}>Total Credits:</Text>
              <Text style={[styles.totalsValue, { color: colors.text }]}>
                ${getTotalCredits().toFixed(2)}
              </Text>
            </View>
            <View style={[styles.totalsRow, styles.balanceRow]}>
              <Text style={[styles.totalsLabel, { color: colors.text, fontWeight: 'bold' }]}>
                Balance:
              </Text>
              <Text style={[
                styles.totalsValue, 
                ...
                {isBalanced() ? 'BALANCED' : `OFF BY $${Math.abs(getTotalDebits() - getTotalCredits()).toFixed(2)}`}
              </Text>
            </View>
          </View>
        </ScrollView>
