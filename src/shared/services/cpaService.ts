...
import { Profile, ClientDashboardData, ClientListItem, } from '@/shared/types';
...
export const getAssignedClients = async (cpaId: string): Promise<ClientListItem[]> => {
    ...
    return (data || [])
        .map((item: { client: any }): ClientListItem | null => {
        ...
        .filter((item: ClientListItem | null): item is ClientListItem => item !== null);
};
