...
const SUBSCRIPTION_SCAN_DAYS = 90;
const FORECAST_MONTHS = 6;
const TREND_THRESHOLD = 100;
const HEALTH_SCORE_BASE = 40;
const HEALTH_SCORE_BONUS = 15;
const CONFIDENCE_DETECTED = 0.8;
...
const ensureProfileExists = async (userId: string): Promise<void> => {
  try {
    ...
    if (!data) {
      console.log(
        `[DataService] 🛠️ Auto-healing: Creating missing profile for ${userId}...`
      );
      ...
      if (insertError && insertError.code !== '23505') {
        console.error(
          '[DataService] ❌ Critical: Failed to heal profile:',
          insertError
        );
      }
    }
...
export const createOrganization = async (name: string, ownerId: string) => {
  ...
  if (memberError)
    console.error('Critical: Created org but failed to add owner', memberError);
  logAuditAction(
    ownerId,
    'ORG_CREATE',
...
export const getExpenseRequests = async (
  ...
  const { data, error } = await supabase
    ...
    .select(
      `*, requester:profiles!expense_requests_requester_id_fkey(first_name, last_name, avatar_url)`
    )
...
export const approveExpenseRequest = async (
  ...
  createNotification(
    request.requester_id,
    'Request Approved',
    `Your expense for ${request.merchant} was approved.`,
    'system'
  );
  logAuditAction(
    approverId,
    'EXPENSE_APPROVE',
    `Approved ${request.amount} for ${request.merchant}`,
...
/**
 * Scans historical transactions for recurring subscription patterns.
 * @param userId - The user ID to scan subscriptions for
 * @returns Array of detected subscriptions
 */
export const scanForSubscriptions = async (
  userId: string
): Promise<DetectedSubscription[]> => {
  if (!userId) throw new ValidationError('userId', userId);

  const ninetyDaysAgo = dayjs()
    .subtract(SUBSCRIPTION_SCAN_DAYS, 'day')
    .toISOString();
  ...
  if (error) {
    console.warn('[SubscriptionScan] Query error:', error.message);
    return [];
  }
...
export const addSubscription = async (
  userId: string,
  sub: Partial<DetectedSubscription>
) => {
...
export const processVoiceTransaction = async (
  ...
  try {
    ...
    const { data: transcriptData, error: fnError } =
      await supabase.functions.invoke('process-voice', {
        body: { audioUrl: publicUrl },
      });

    if (fnError || !transcriptData?.text) {
      console.warn('[AI] Cloud transcription failed, returning mock.');
      return processNaturalLanguageTransaction(
        userId,
        'Spent 25 dollars at Voice Demo Lunch'
      );
    }
...
export const processNaturalLanguageTransaction = async (
  userId: string,
  input: string
) => {
  const prompt = `
  ...
  try {
    ...
    if (!data.amount || !data.merchant)
      throw new Error('Could not detect amount or merchant.');
...
export const getSpendingForecast = async (
  userId: string
): Promise<{ predictedAmount: number; trend: 'up' | 'down' | 'stable' }> => {
  if (!userId) throw new ValidationError('userId', userId);

  const sixMonthsAgo = dayjs().subtract(FORECAST_MONTHS, 'month').toISOString();
  ...
  if (error) {
    console.warn('[SpendingForecast] Query error:', error.message);
    return { predictedAmount: 0, trend: 'stable' };
  }
  ...
  transactions.forEach((t) => {
    ...
    monthlySpend[monthKey] =
      (monthlySpend[monthKey] || 0) + Math.abs(Number(t.amount));
  });
...
export const getFinancialHealthScore = async (
  ...
  if (summary.income === 0) return HEALTH_SCORE_BASE;

  const savingsRate = (summary.income - summary.expense) / summary.income;
  let score = savingsRate * 100;
  if (summary.balance > 0) score += HEALTH_SCORE_BONUS;
...
export const createBudget = async (
  ...
  if (!catId) {
    const { data: newCat } = await supabase
      .from('categories')
      .insert({ name: categoryName, type: 'expense', user_id: userId })
      .select()
      .single();
    catId = newCat?.id;
  }
...
export const getFinancialSummary = async (
  ...
  if (!transactions)
    return { balance: 0, income: 0, expense: 0, savings_rate: 0, trend: [] };
...
export const pickAndUploadFile = async (userId: string) => {
  try {
    ...
    if (asset.size && asset.size > APP_CONFIG.MAX_FILE_SIZE_MB * 1024 * 1024) {
      throw new Error(
        `File size exceeds ${APP_CONFIG.MAX_FILE_SIZE_MB}MB limit`
      );
    }
...
export const getClientCpas = async (clientId: string) => {
  ...
  return data.map((item: any) => ({
    id: item.cpa.id,
    name: item.cpa.first_name
      ? `${item.cpa.first_name} ${item.cpa.last_name || ''}`.trim()
      : 'CPA',
...
export const requestCPA = async (userId: string, cpaIdOrEmail: string) => {
  ...
  const { data: existing } = await supabase
    .from('cpa_clients')
    .select('id')
    .match({ client_id: userId, cpa_id: cpaId })
    .maybeSingle();
  if (existing) throw new Error('Connection already exists.');

  const { error } = await supabase
    .from('cpa_clients')
    .insert({ client_id: userId, cpa_id: cpaId, status: 'pending' });
  if (error) throw error;
...
export const inviteClient = async (cpaId: string, clientEmail: string) => {
  ...
  const { data: existing } = await supabase
    .from('cpa_clients')
    .select('id')
    .match({ client_id: client.id, cpa_id: cpaId })
    .maybeSingle();
  if (existing)
    throw new Error('Invitation already sent or user is already a client.');

  const { error: inviteError } = await supabase
    .from('cpa_clients')
    .insert({ client_id: client.id, cpa_id: cpaId, status: 'pending' });
...
export const acceptCpaClient = async (cpaId: string, clientId: string) => {
  ...
  await createNotification(
    clientId,
    'Request Accepted',
    'Your CPA accepted your request.',
    'cpa',
    cpaId
  );
};
...
export const updateTransactionTaxStatus = async (
...
export const getTicketDetails = async (ticketId: string) => {
  const { data, error } = await supabase
    .from('tickets')
    .select(
      `*, messages:ticket_messages(*), user:profiles!tickets_user_id_fkey(*)`
    )
    .eq('id', ticketId)
    .single();
...
export const getActiveCPAs = async (): Promise<User[]> => {
  ...
  const filteredData = (data || []).filter((p: any) => {
    ...
    if (
      ...
      (fullName.length < 3 || fullName === 'user' || fullName === 'cpa')
    ) {
      return false;
    }
...
export const linkDocumentToTransaction = async (
  documentId: string,
  transactionId: string
) => {
  const { error } = await supabase
...
export const unlinkDocumentFromTransaction = async (documentId: string) => {
  const { error } = await supabase
    .from('documents')
...
export const getDocumentsForTransaction = async (
  transactionId: string
): Promise<DocumentItem[]> => {
  const { data, error } = await supabase
    .from('documents')
...
export const notifyStaffNewTicket = async (
  ticketId: string,
  subject: string,
  userName: string
) => {
...
export const notifyUserTicketUpdate = async (
  userId: string,
  ticketId: string,
  status: string
) => {
...
export const notifyCpaRequest = async (
  cpaId: string,
  clientName: string,
  createdBy?: string
) => {
  await createNotification(
    cpaId,
    'New CPA Request',
    `${clientName} has requested access.`,
    'cpa',
    undefined,
    createdBy
  );
...
export const notifyClientInvitation = async (
  clientId: string,
  cpaName: string,
  createdBy?: string
) => {
