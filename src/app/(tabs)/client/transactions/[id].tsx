    ...
    <View style={[styles.itemContainer, { backgroundColor: colors.surface }]}>
      ...
      >
        {isIncome ? (
          ...
          <ArrowDown color={colors.error} size={20} />
        )}
      </View>
...
const FilterControls = ({ onFilterChange, onSortChange, colors }: any) => {
  ...
  const handleSort = (newSortBy: SortBy) => {
    const newSortDir =
      sortBy === newSortBy ? (sortDir === 'asc' ? 'desc' : 'asc') : 'desc';
    setSortBy(newSortBy);
      ...
      >
        <Text
          style={[
            styles.filterText,
            { color: colors.text },
            filter === 'all' && { color: colors.primaryContrast },
          ]}
        >
          All
        </Text>
      ...
      >
        <Text
          style={[
            styles.filterText,
            { color: colors.text },
            filter === 'income' && { color: colors.primaryContrast },
          ]}
        >
          Income
        </Text>
      ...
      >
        <Text
          style={[
            styles.filterText,
            { color: colors.text },
            filter === 'expense' && { color: colors.primaryContrast },
          ]}
        >
          Expense
        </Text>
      ...
      >
        ...
        {sortBy === 'date' && (
          <ChevronDown
            color={colors.text}
            size={16}
            style={{ transform: [{ scaleY: sortDir === 'asc' ? 1 : -1 }] }}
          />
        )}
      </TouchableOpacity>
    </View>
...
export default function ClientTransactionsScreen() {
  ...
  return (
    <ScreenContainer>
      <Stack.Screen
        options={{ title: `${client?.name}'s Transactions` || 'Transactions' }}
      />
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TransactionListItem item={item} colors={colors} />
        )}
