  ...
  React.useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const currentAssets = sheetData.accounts.filter(
        ...
        const totalFixedAssets = sheetData.assets.reduce(
          (sum, a) => sum + a.value,
          0
        );
        const totalAssets = totalCurrentAssets + totalFixedAssets;

        const totalLiabilities = sheetData.liabilities.reduce(
          (sum, l) => sum + l.balance,
          0
        );
...
const ReportRow = ({
  ...
  >
    ...
    <Text
      style={[
        styles.rowValue,
        { fontWeight: isTotal ? 'bold' : 'normal', color: colors.text },
      ]}
    >
    ...
    <ScreenContainer>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        >
          ...
          {data.currentAssets.map((asset: Account) => (
            <ReportRow
              key={asset.id}
              label={asset.name}
              value={asset.balance}
          ...
          {data.assets.map((asset: FixedAsset) => (
            <ReportRow
              key={asset.id}
              label={asset.name}
              value={asset.value}
          ...
          {data.liabilities.map((lib: Liability) => (
            <ReportRow
              key={lib.id}
              label={lib.name}
              value={lib.balance}
