    ...
    const ApiKeyInput = ({ label, value, onChangeText, provider }: any) => (
      <View style={styles.inputContainer}>
        ...
        <View style={styles.inputRow}>
          <KeyRound color={colors.textSecondary} size={20} style={styles.keyIcon} />
          <TextInput
        ...
        {testResult[provider] === 'success' && (
          <View style={styles.resultContainer}>
            ...
            <Text style={[styles.resultText, { color: colors.success }]}>Connection successful!</Text>
          </View>
        )}
        {testResult[provider] === 'error' && (
          <View style={styles.resultContainer}>
            <AlertTriangle color={colors.error} size={16} />
            <Text style={[styles.resultText, { color: colors.error }]}>Connection failed. Please check your key.</Text>
          </View>
        )}
      </View>
    );
    ...
    return (
      <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
        <View style={styles.form}>
          <ApiKeyInput label="OpenAI" value={openAIKey} onChangeText={setOpenAIKey} provider="openai" />
          <ApiKeyInput label="Google Gemini" value={geminiKey} onChangeText={setGeminiKey} provider="gemini" />
          <ApiKeyInput label="Anthropic Claude" value={claudeKey} onChangeText={setClaudeKey} provider="claude" />
        </View>
        <TouchableOpacity style={[styles.saveButton, { backgroundColor: colors.primary }]} onPress={handleSaveKeys}>
