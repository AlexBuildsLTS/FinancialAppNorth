...
const PasswordStrength = ({ password }: { password: string }) => {
  ...
  const Req = ({ met, label }: { met: boolean; label: string }) => (
    <View className='flex-row items-center gap-1 mr-3 mb-1'>
      ...
      <Text className={`text-xs ${met ? 'text-[#64FFDA]' : 'text-[#8892B0]'}`}>{label}</Text>
    </View>
  );

  return (
    <View className='flex-row flex-wrap mt-2'>
      <Req met={hasLength} label='8+ Chars' />
      <Req met={hasUpper} label='Uppercase' />
      <Req met={hasNum} label='Number' />
      <Req met={hasSpec} label='Symbol' />
    </View>
  );
};
