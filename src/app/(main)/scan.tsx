...
import { uploadDocument } from '../../services/dataService'; // Use Real Service
...
export default function ScanScreen() {
  ...
  const handleUpload = async () => {
    ...
    setIsUploading(true);
    try {
      await uploadDocument(user.id, photo, `scan_${Date.now()}.jpg`, 'receipt');
      ...
      {!photo ? (
        ...
        >
          <SafeAreaView className="flex-1 justify-between p-6">
            <View className="flex-row justify-between items-center">
              ...
              >
                <Zap size={20} color={flash === 'on' ? '#0A192F' : 'white'} />
              </TouchableOpacity>
            </View>

            <View className="absolute inset-0 items-center justify-center pointer-events-none">
              <View className="w-64 h-96 border-2 border-[#64FFDA]/50 rounded-2xl" />
            </View>
        ...
        <SafeAreaView className="flex-1 bg-[#0A192F] p-6">
          ...
          <View className="flex-row gap-4">
            ...
            <TouchableOpacity onPress={handleUpload} disabled={isUploading} className="flex-1 py-4 bg-[#64FFDA] rounded-xl items-center">
              <Text className="text-[#0A192F] font-bold">{isUploading ? 'Uploading...' : 'Use Photo'}</Text>
            </TouchableOpacity>
          </View>
        </SafeAreaView>
      )}
