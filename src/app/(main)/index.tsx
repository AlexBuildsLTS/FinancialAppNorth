...
const StatCard = ({ title, value, subtitle, icon: Icon, color, link, router, style }: any) => (
  <TouchableOpacity 
  ...
  >
    ...
    {subtitle && <Text className="text-[#8892B0] text-xs mt-1">{subtitle}</Text>}
  </TouchableOpacity>
);
...
export default function Dashboard() {
  ...
  const [forecast, setForecast] = useState<{ predictedAmount: number, trend: 'up' | 'down' | 'stable' }>({ predictedAmount: 0, trend: 'stable' });
  ...
  const fetchData = async (showLoader = false) => {
    ...
    try {
      ...
      const [txs, summary, budgetData, forecastData, subData] = await Promise.all([
    ...
    >
      ...
      {subscriptions.length > 0 && (
        <View className="bg-[#112240] rounded-2xl border border-white/5 p-4 mb-6 flex-row items-center justify-between">
          ...
          <TouchableOpacity onPress={() => router.push('/(main)/finances/budgets')}>
      ...
      <View className="bg-[#112240] rounded-2xl border border-white/5 p-6 mb-6">
        ...
        <View className="flex-row flex-wrap gap-4">
          ...
          >
            ...
            <Text className="mt-2 text-sm font-medium text-white">Send Money</Text>
          </TouchableOpacity>
          ...
          >
            ...
            <Text className="mt-2 text-sm font-medium text-white">Scan Receipt</Text>
          </TouchableOpacity>
          ...
          >
            ...
            <Text className="mt-2 text-sm font-medium text-white">Add Budget</Text>
          </TouchableOpacity>
        </View>
      </View>

      {/* Charts Section */}
      <View className={`mb-6 ${isDesktop ? 'flex-row gap-6' : 'flex-col gap-6'}`}>
        ...
        {loading ? (
          ...
          <View className="gap-4">
            {transactions.slice(0, 5).map((tx) => (
              <View key={tx.id} className="flex-row items-center gap-4 pb-3 border-b border-white/5 last:border-0">
                <View className={`w-2 h-2 rounded-full ${Number(tx.amount) >= 0 ? 'bg-[#64FFDA]' : 'bg-[#F472B6]'}`} />
                <View className="flex-1">
                  <Text className="font-medium text-white">{tx.description || 'Unknown'}</Text>
