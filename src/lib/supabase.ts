...
if (!hasAnon) {
  console.warn(
    'Supabase anon client not configured: SUPABASE_URL or SUPABASE_ANON_KEY missing. Certain features will throw when used.'
  );
}
if (!hasService) {
}
...
//
// ## FIX 1: Added { db: { schema: 'northfinance' } } to both clients ##
//
export const supabase: any = hasAnon
  ? createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
      db: { schema: 'northfinance' },
    })
  : makeMissingClient(
      'supabase',
...
/* --------------------------------------------------------------
   2️⃣ Type definitions – mirrors the northfinance schema
   -------------------------------------------------------------- */
...
// This interface now matches the new 'cpa_client_assignments' table
export interface CpaClient {
  id: string; // uuid
  cpa_user_id: string; // uuid
...
export async function getCurrentUser(): Promise<{
  ...
  // This .from('profiles') will now correctly use 'northfinance.profiles'
...
/**
 * Subscribe to *any* northfinance table.
...
export function subscribe<T extends Record<string, any>>(
  ...
  const channel = _sb
    ...
    .on(
      'postgres_changes',
      //
      // ## FIX 2: Changed 'public' to 'northfinance' ##
      //
      { event: '*', schema: 'northfinance', table },
      (payload: any) => {
...
/* --------------------------------------------------------------
   ...
   -------------------------------------------------------------- */
// All .from() calls below will now default to 'northfinance' schema
export const api = {
  ...
  async sendMessage(
    ...
    const { error } = await _sb
      .from('messages')
      .insert([{ channel_id: channelId, user_id: fromUserId, content }]);
    if (error) throw error;
  },
  ...
  async createNotification(userId: string, title: string, body: string) {
    const { error } = await _sb
      .from('notifications')
      .insert([{ user_id: userId, title, body, is_read: false }]);
    if (error) throw error;
  },

  //
  // ## FINAL FIX: Corrected 'cpa_clients' to 'cpa_client_assignments' ##
  //
  // cpa clients
  async linkClient(cpaId: string, clientId: string) {
    const { error } = await _sb
      .from('cpa_client_assignments') // <-- Table name fixed
      .insert([
        {
          cpa_user_id: cpaId,
  ...
  async listCpaClients(cpaId: string) {
    const { data, error } = await _sb
      .from('cpa_client_assignments') // <-- Table name fixed
      .select('*')
      .eq('cpa_user_id', cpaId); // <-- Column name fixed
    if (error) throw error;
    return data ?? [];
  },
};

// admin helpers use _sbAdmin;
export async function adminBroadcast(
  title: string,
  body: string,
  targetRoles: Role[] = ['member', 'premium', 'cpa', 'support']
) {
  const { data: users, error } = await _sbAdmin
    .from('profiles')
    .select('id')
    .in('role', targetRoles);
  if (error) throw error;
  ...
  const { error: notifErr } = await _sbAdmin
    .from('notifications')
    .insert(inserts);
...
export async function adminChangeUserRole(
  ...
  if (!resp.ok) {
    ...
    //
    // ## FIX 3: Removed the stray '.' character ##
    //
...
export async function getMonthlyIncomeSummary(
  ...
  // This will now call 'northfinance.get_monthly_income_summary'
